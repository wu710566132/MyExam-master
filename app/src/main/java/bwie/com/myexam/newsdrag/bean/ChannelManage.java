package bwie.com.myexam.newsdrag.bean;
...
import bwie.com.myexam.Bean;
import bwie.com.myexam.MyPageAdapter;
import bwie.com.myexam.newsdrag.dao.ChannelDao;
		...
		x.http().post(parms, new Callback.CommonCallback<String>() {
			...
			public void onSuccess(String result) {
				...
				for (int i=0;i<category.size();i++){
					defaultUserChannels.add(new ChannelItem(i, category.get(i).getName(),i, 1));

				}
				for (int i=0;i<nations.size();i++){
					defaultOtherChannels.add(new ChannelItem(i+category.size(), nations.get(i).getName(), i, 0));
